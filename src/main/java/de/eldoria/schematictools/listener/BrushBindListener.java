...
public class BrushBindListener implements Listener {
    ...
    public void unbindTool(Player player, ItemStack stack) {
        ...
        try {
            if (getLocalSession(player).getTool(BukkitAdapter.adapt(stack).getType()) instanceof BrushTool tool
                ...
                messageSender.sendActionBar(player, I18n.get().resolveFormatted("action.tool.deactivated"));
            }
    ...
    public void bindTool(Player player, ItemStack stack) {
        ...
        if (optTool.isEmpty()) {
            plugin.getLogger().warning(I18n.get().resolve("error.tool.notexist", toolMeta, player.getName()));
            if (configuration.toolRemoval().isRemoveInvalidTools()) {
                messageSender.sendActionBar(player, I18n.get().resolveFormatted("action.tool.broke"));
                stack.setAmount(0);
            }
            return;
        }
        ...
        if (tool.hasUsage() && toolMeta.usages() >= tool.usages()) {
            if (configuration.toolRemoval().isRemoveUsed()) {
                messageSender.sendActionBar(player, I18n.get().resolveFormatted("action.tool.usages.exhausted"));
                stack.setAmount(0);
            }
            return;
        }
        ...
        tool.getBrush(sbr.storageRegistry().activeStorage())
                .thenAccept(brush -> {
                    if (brush.isEmpty()) {
                        plugin.getLogger().warning(I18n.get().resolve("error.tool.invalidbrush", tool, player.getName()));
                    ...
                    messageSender.sendActionBar(player, I18n.get().resolveFormatted("action.tool.activated"));
                });
    }
}
