...
public class Bind extends AdvancedCommand implements IPlayerTabExecutor {
    ...
    public void onCommand(@NotNull Player player, @NotNull String alias, @NotNull Arguments args) throws CommandException {
        ...
        var tool = byName.get();
        SchematicTool.initTool(hand, tool);
        // Persist the item metadata (e.g. the recorded base material) written during init.
        configuration.save();

        messageSender().sendMessage(player, MessageComposer.create()
                .text(I18n.get().resolveFormatted("commands.bind.bound")));
    }
